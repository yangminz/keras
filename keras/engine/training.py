...
'''
    (Pdb) p self.optimizer
    <keras.optimizers.RMSprop object at 0xad065c4c>
    (Pdb) p self.sample_weight_mode
    None
    (Pdb) p self.loss
    'binary_crossentropy'
    (Pdb) p self.loss_weights
    None
'''
...
'''
    ...
    [<function binary_crossentropy at 0xad55d584>]
    (Pdb) p weighted_losses
    [<function weighted_objective.<locals>.weighted at 0xad01fecc>]
'''
...
'''
    (Pdb) p sample_weights
    [dense_3_sample_weights]
    (Pdb) p sample_weights[0]._keras_shape
    (None,)
    (Pdb) p sample_weight_modes
    [None]
'''
...
'''
- `keras/engine/training.py: def weighted_objective(fn): def weighted()`
'''
...
返回一个Theano变量:
    ...
    (Pdb) p output_loss.type
    TensorType(float32, scalar)
    (Pdb) p output_loss.shape
    Shape.0
'''
...
'''
在theano后端下, `total_loss`是一个`TensorType`

    (Pdb) p total_loss.type
    TensorType(float32, scalar)
    (Pdb) p total_loss.shape
    Shape.0
    (Pdb) p total_loss.ndim
    0
'''
...
'''
`y_true`和`y_pred`都是theano的`TensorType`, 而非`TensorVariable`
...
'''
    (Pdb) p self.optimizer
    <keras.optimizers.RMSprop object at 0xacfe458c>
    (Pdb) p self.total_loss
    Elemwise{mul,no_inplace}.0
    (Pdb) p self.total_loss.ndim
    0
    (Pdb) p self.sample_weights
    [dense_3_sample_weights]
    (Pdb) p self.sample_weights[0].type
    TensorType(float32, vector)
'''
...
'''
    (Pdb) p self.trainable_weights
    [dense_1_W, dense_1_b, dense_2_W, dense_2_b, dense_3_W, dense_3_b]
    
    (Pdb) p self.trainable_weights[0].type
    TensorType(float32, matrix)
    (Pdb) p self.trainable_weights[0].ndim
    2
    (Pdb) p self.trainable_weights[0]._keras_shape
    (20, 64)
    (Pdb) p self.trainable_weights[0].get_value()
    array([[  4.36547101e-02,   2.56271362e-01,   1.11427486e-01, ...,
              8.87558088e-02,   9.93706062e-02,  -2.11460013e-02],
           [  2.25955693e-04,  -8.97000432e-02,  -2.03371838e-01, ...,
              2.48232245e-01,   1.35912985e-01,  -2.44565502e-01],
           [  3.35906819e-02,  -1.05202422e-01,   3.39145660e-02, ...,
              2.23663583e-01,   5.52745126e-02,  -2.92096240e-03],
           ..., 
           [  1.79936141e-01,   8.18167776e-02,   6.06339127e-02, ...,
              1.29919395e-01,   2.26625204e-01,   8.59595761e-02],
           [  2.66311318e-01,   4.53082547e-02,  -6.40599579e-02, ...,
             -2.13101804e-01,  -2.49754891e-01,  -1.59356207e-01],
           [ -5.09282537e-02,   1.88218534e-01,  -1.27846673e-01, ...,
              1.03319474e-01,   2.53403664e-01,   1.15465768e-01]], dtype=float32)

    (Pdb) p self.trainable_weights[0].ndim
    2
    (Pdb) p self.trainable_weights[2]._keras_shape
    (64, 64)
    (Pdb) p self.trainable_weights[2].get_value()
    array([[ 0.07498681, -0.15829445,  0.00266682, ...,  0.13126893,
             0.16091387, -0.05538922],
           [-0.19389297,  0.12210862, -0.03000132, ...,  0.18167147,
            -0.05566167,  0.10342129],
           [-0.16935679,  0.21618675,  0.16768499, ...,  0.17678811,
            -0.13090572,  0.15496668],
           ..., 
           [ 0.00982143,  0.1919228 , -0.17170633, ...,  0.01070409,
             0.0346653 , -0.17682637],
           [-0.02499515,  0.04590354, -0.02160676, ..., -0.21364406,
             0.20679718,  0.12813476],
           [-0.19313639,  0.09237145,  0.14627795, ..., -0.05365273,
             0.015907  , -0.07350719]], dtype=float32)
'''
...
'''
这里的都是theano中不带具体数据的变量`matrix`和`vector`, 构成一个算子, 从输入`input_1`到输出`dense_3_target`
    ...
    (Pdb) p inputs[0].type
    TensorType(float32, matrix)
    (Pdb) p inputs[0]._keras_shape
    (None, 20)

    (Pdb) p inputs[1].type
    TensorType(float32, matrix)    
    (Pdb) p inputs[1]._keras_shape
    (None, None)

    (Pdb) p inputs[2].type
    TensorType(float32, vector)    
    (Pdb) p inputs[2]._keras_shape
    (None,)

    (Pdb) p self._collected_trainable_weights
    [dense_1_W, dense_1_b, dense_2_W, dense_2_b, dense_3_W, dense_3_b]
    (Pdb) p self.constraints
    {}

通过`self.optimizer.get_updates()`得到`(v, nv)`的`updates`元组列表
构建好梯度的形式计算, 或者说梯度的函数
'''
            training_updates = self.optimizer.get_updates(self._collected_trainable_weights,
                                                          ...
                                                          self.total_loss)
'''
    (Pdb) p training_updates
    [(<TensorType(float32, matrix)>, Elemwise{add,no_inplace}.0), (dense_1_W, Elemwise{sub,no_inplace}.0), (<TensorType(float32, vector)>, Elemwise{add,no_inplace}.0), (dense_1_b, Elemwise{sub,no_inplace}.0), (<TensorType(float32, matrix)>, Elemwise{add,no_inplace}.0), (dense_2_W, Elemwise{sub,no_inplace}.0), (<TensorType(float32, vector)>, Elemwise{add,no_inplace}.0), (dense_2_b, Elemwise{sub,no_inplace}.0), (<TensorType(float32, matrix)>, Elemwise{add,no_inplace}.0), (dense_3_W, Elemwise{sub,no_inplace}.0), (<TensorType(float32, vector)>, Elemwise{add,no_inplace}.0), (dense_3_b, Elemwise{sub,no_inplace}.0)]
    (Pdb) p len(training_updates)
    12
    (Pdb) p len(self.updates)
    0
    (Pdb) p len(updates)
    12
'''
...
'''
构建算子. 每次调用算子时, 返回误差`loss`与度量`metrics`, 同时对所有权重进行更新!
'''
...
'''
*计算的关键所在!*
返还的结果是误差`loss`、精度`acc`二元组, *同时已经完成了权重更新!*
这样形式的调用要多亏`keras/backend/theano_backend.py: class Function(object): def __call__()`
'''
                outs = f(ins_batch)
'''
    (Pdb) p outs
    [array(9.465790748596191, dtype=float32), array(0.40625, dtype=float32)]

误差`loss`、精度`acc`在这里根据批次重新计算一次
'''
...
`batch_logs['loss']`是在这里添加的
'''
                for l, o in zip(out_labels, outs):
                    batch_logs[l] = o
'''
循环结束以后, `batch_logs`被添加了`out_labels`上所有的内容

    (Pdb) p batch_logs
    {'batch': 0, 'size': 32, 'loss': array(9.465790748596191, dtype=float32), 'acc': array(0.40625, dtype=float32)}

这里`batch_logs['loss'][0]`就是本回epoch的误差大小
调用的`callbacks.on_batch_end()`函数在：
    keras/callbacks.py: class CallbackList(object): def on_batch_end()
在`callbacks.on_batch_end()`中将对用户打印误差、进度等信息
'''
...
'''
调用的`callbacks.on_batch_end()`函数在：
    keras/callbacks.py: class CallbackList(object): def on_epoch_end()
'''
...
'''
调用的`callbacks.on_batch_end()`函数在：
    keras/callbacks.py: class CallbackList(object): def on_train_end()
'''
...
返还算子, 构建theano计算框架. *每次调用算子时, 返回误差`loss`与度量`metrics`, 同时对所有权重进行更新!*
'''
        self._make_train_function()
        f = self.train_function
'''
因为有`keras/backend/theano_backend.py: class Function(object): def __call__()`
所以每次根据输入就可以直接计算`loss`, `metrics`, 同时更新权重:
