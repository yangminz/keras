...
'''
根据`initializer`选择初始化方法, 初始化权重`weight`
默认是`keras/initializations.py: glorot_uniform()`
...
'''
    (Pdb) p self._trainable_weights
    []

以theano为后端, 返回的backend变量, `weight`, 其粗略的使用方法是这样的:

    (Pdb) p weight
    dense_1_W
    (Pdb) p weight._keras_shape
    (20, 64)
    (Pdb) p weight._uses_learning_phase
    False
    (Pdb) p weight.get_value()
    array([[-0.18000209, -0.17224771, -0.12359736, ..., -0.22155757,
            -0.04562056, -0.08109311],
           [ 0.11669444, -0.20815612,  0.12445661, ..., -0.14419951,
            -0.24703567, -0.00051669],
           [ 0.14744765,  0.15038189,  0.01626701, ..., -0.02329718,
             0.19016084, -0.13313192],
           ..., 
           [ 0.07991317,  0.08292518, -0.2456329 , ..., -0.04320575,
            -0.11614873, -0.00398963],
           [-0.02188518,  0.18602338, -0.22873425, ...,  0.08094199,
            -0.02277661,  0.02456222],
           [-0.10471732,  0.24119478, -0.16947378, ...,  0.02106747,
             0.02769423,  0.03945029]], dtype=float32)
'''
...
'''
到这个函数结束, 已经根据输入的`shape`和`initializer`得到了一个backend变量
并且根据`trainable`将此backend变量存放到恰当的权重列表中了

    (Pdb) p self._trainable_weights
    [dense_1_W]
    (Pdb) p self._non_trainable_weights
    []
'''
        return weight
...
'''
在特异化层, 例如`Dense`, 初始化时需要同时调用`__init__`和`__call__`
'''
...
`input_1`是functional API在`Dense_1`之前创建的层
作为输入传入`Dense`:
...
接下来跳转到成员函数`self.build()`
`build()`是多态, 会被子类的`build`覆盖
'''
            ...
            self.built = True
'''
到此为止, backend变量类型的权重矩阵已经`build()`完成了, 可以查看:
    ...
    array([[-0.19744697, -0.23982006,  0.04885469, ..., -0.2529999 ,
             0.24717741, -0.26054648],
           [-0.17473149,  0.02087193, -0.04347471, ..., -0.26710355,
            -0.0192588 , -0.02679542],
           [-0.05444423, -0.23376492, -0.13434404, ...,  0.09269784,
            -0.01709896,  0.11424621],
           ..., 
           [ 0.03649697, -0.02483479, -0.06361866, ..., -0.05084773,
            -0.03598849, -0.11858702],
           [ 0.25795484, -0.20457716, -0.10942136, ..., -0.24317898,
             0.09041268, -0.03245977],
           [ 0.174127  ,  0.09449331, -0.23126003, ..., -0.04347713,
            -0.20868103, -0.20215937]], dtype=float32)
'''
...
'''
`Input`层传来的张量, 作为`dense_1`的输入

    (Pdb) p input_tensors
    [input_1]
    (Pdb) p input_tensors[0]._keras_shape
    (None, 20)
'''
        inbound_layers = []
        node_indices = []
...
索引的情况:
    ...
    (Pdb) p inbound_layers
    [<keras.engine.topology.InputLayer object at 0xb6e2674c>]
    (Pdb) p node_indices
    [0]
    (Pdb) p tensor_indices
    [0]
'''
        if inbound_layers:
...
'''
    (Pdb) p self.inbound_nodes
    [<keras.engine.topology.Node object at 0xacfd59ac>]
    (Pdb) p outputs
...
'''
        ...
        if isinstance(output, (list, tuple)):
            ...
            self.outputs = [output]
'''
占位符形式的变量`TensorVariable`, 这是后端自有的数据形式, 例如theano
网络全体的输入与输出张量

    (Pdb) p self.inputs
    [input_1]
    (Pdb) p self.inputs[0]._keras_shape
    (None, 20)
    (Pdb) p self.outputs
    [Softmax.0]
    (Pdb) p self.outputs[0]._keras_shape
    (None, 1)
'''
...
'''
这几个是计算缓冲. 从输入计算到输出的调用如下:
- `def call()`
    缓冲在这里, 如果命中缓冲就不需要再调用`run_internal_graph`
    - `def run_internal_graph()`
        主要的计算任务在这个函数里, 计算代价比较高
'''
...
'''
    (Pdb) p layer
    <keras.engine.topology.InputLayer object at 0xb6ebd74c>
    (Pdb) p node_index
    0
    (Pdb) p tensor_index
    0
...
'''
    (Pdb) p layer
    <keras.layers.core.Dense object at 0xad06b38c>
    (Pdb) p node_index
    0
    (Pdb) p tensor_index
    0
...
'''
    (Pdb) p mask_cache_key
    '3069010060_139286256'
'''
        masks = []
        for x in self.outputs:
            layer, node_index, tensor_index = x._keras_history
...
'''
    (Pdb) p self._output_mask_cache
    {'3069010060_139286256': None}
'''
...
'''
    (Pdb) p self.input_layers
    [<keras.engine.topology.InputLayer object at 0xb6ebd74c>]
    (Pdb) p self.input_layers_node_indices
    [0]
    (Pdb) p self.input_layers_tensor_indices
    [0]
'''
...
'''
    (Pdb) p self.input_names
    ['input_1']
    (Pdb) p self.output_names
    ['dense_3']
'''
...
'''
    (Pdb) p self.internal_input_shapes
    [(None, 20)]
    (Pdb) p self.internal_output_shapes
    [(None, 1)]
'''
...
'''
    (Pdb) p tensor
    Softmax.0
    (Pdb) p tensor._keras_history
    (<keras.layers.core.Dense object at 0xad06b38c>, 0, 0)
'''
            node = layer.inbound_nodes[node_index]
...
- 关于结点`Node`的信息
    这里的结点是输出层`dense_3`的输入结点, 输出层是没有输出结点的, 因为它不再向上连接网络:

    (Pdb) p node
    <keras.engine.topology.Node object at 0xad06d3ec>
    (Pdb) p layer.inbound_nodes
    [<keras.engine.topology.Node object at 0xad06d3ec>]
    (Pdb) p layer.outbound_nodes
    []
    (Pdb) p node_key
    'dense_3_ib-0'
...
- 输出层`dense_3`的信息
    ...
    'dense_3'
    (Pdb) p layers_depths
    {<keras.layers.core.Dense object at 0xad06b38c>: 0}
    (Pdb) p previously_seen_depth
    None
    (Pdb) p current_depth
    0

- 网络模型信息:
    将`dense_3`的结点`'dense_3_ib-0'`添加到`seen_nodes`

    (Pdb) p seen_nodes
    {'2902905836-0'}
    (Pdb) p id(node)
    2902905836
    (Pdb) p container_nodes
    {'dense_3_ib-0'}
    (Pdb) p node_depth
    None
    (Pdb) p nodes_depths
    {<keras.engine.topology.Node object at 0xad06d3ec>: 0}
'''
...
'''
        ...
        for x in self.outputs:
            seen_nodes = set()
'''
"自顶向下"地建网络, 遍历输出层, 向输入层递归, 将网络所需要的所有`Node`加入`seen_nodes`
`x`是输出层张量, 数据记录在`x._keras_history`中

    (Pdb) p x
    Softmax.0
    (Pdb) p x._keras_history
    (<keras.layers.core.Dense object at 0xad06b38c>, 0, 0)
    (Pdb) p x._keras_history[0].name
    'dense_3'
'''
            build_map_of_graph(x, seen_nodes, depth=0)
'''
完成构建图以后, 图中所有的`Node`为:
...
'''
    (Pdb) p layers_by_depth
    {0: [<keras.layers.core.Dense object at 0xad06b38c>], 
     1: [<keras.layers.core.Dense object at 0xad0d710c>], 
     2: [<keras.layers.core.Dense object at 0xadcb4ecc>], 
     3: [<keras.engine.topology.InputLayer object at 0xb6ebd74c>]}
'''
...
(Pdb) p self.layers
[<keras.engine.topology.InputLayer object at 0xb6ebd74c>,
 <keras.layers.core.Dense object at 0xadcb4ecc>,
 <keras.layers.core.Dense object at 0xad0d710c>, 
 <keras.layers.core.Dense object at 0xad06b38c>]
(Pdb) p self.layers_by_depth
{0: [<keras.layers.core.Dense object at 0xad06b38c>],
 1: [<keras.layers.core.Dense object at 0xad0d710c>], 
 2: [<keras.layers.core.Dense object at 0xadcb4ecc>], 
 3: [<keras.engine.topology.InputLayer object at 0xb6ebd74c>]}
...
'''
    (Pdb) p computable_tensors
    [input_1, input_1, Elemwise{mul,no_inplace}.0, Elemwise{mul,no_inplace}.0, Softmax.0]
    (Pdb) p layers_with_complete_input
    ['input_1', 'dense_1', 'dense_2', 'dense_3']
    (Pdb) p self.container_nodes
    {'input_1_ib-0', 'dense_3_ib-0', 'dense_2_ib-0', 'dense_1_ib-0'}
'''
...
'''
    (Pdb) p self.outbound_nodes
    []
    (Pdb) p self.inbound_nodes
    [<keras.engine.topology.Node object at 0xad068cec>]
    (Pdb) p self.inbound_nodes[0].outbound_layer.name
    'model_1'
    (Pdb) p self.inbound_nodes[0].inbound_layers
    []
'''
        self.built = True
        self.supports_masking = False
