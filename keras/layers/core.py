...
'''
    (Pdb) p self.init
    <function glorot_uniform at 0xadae126c>
    (Pdb) p self.activation
    <function relu at 0xad0e51dc>
    (Pdb) p self.output_dim
    64
    (Pdb) p self.input_dim
    None
'''
        ...
        self.input_spec = [InputSpec(ndim='2+')]
'''
在没指定权重的情况下, 初始化为`None`

    (Pdb) p self.initial_weights
    None
    (Pdb) p self.input_spec[0]
    <keras.engine.topology.InputSpec object at 0xad0a772c>
'''
        if self.input_dim:
            kwargs['input_shape'] = (self.input_dim,)
'''
反过来初始化父类作为自己的初始化!
父类为`keras/engine/topology.py: class Layer(object)`
会依次执行`Layer`的两个函数：
- `__init__`
- `__call__`
    - `self.build()` # 子类的多态函数
两个函数完成以后,就结束`Dense`层的创建, 返回实验室代码
'''
        super(Dense, self).__init__(**kwargs)

    def build(self, input_shape):
'''
会在父类`Layer.__call__()`中作为多态被调用
主要实现了*权重的定义*
'''
...
添加权重函数`add_weight`的参数情况:
    ...
    64
    (Pdb) p self.init
    <function glorot_uniform at 0xadb5026c>
    (Pdb) p self.W_regularizer
    None
    (Pdb) p self.W_constraint
    None
    (Pdb) p self.input_spec
    [<keras.engine.topology.InputSpec object at 0xb6edfd0c>]

随后调用`keras/engine/topology.py: class Layer(object): def add_weight()`
根据输入输出的形状`input_dim`, `self.output_dim`和初始化方法`self.init`
创造了一个backend变量, 并且将它放在恰当的list中了:
